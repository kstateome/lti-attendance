...
public class AttendanceSummaryCSVService {
    ...
    * */
    public  StringBuilder createAttendanceSummaryCsv(boolean isSimpleAttendance, List<AttendanceSummaryModel> summaryForSections) throws IOException {
        ...
        List<String> headers = isSimpleAttendance ? Arrays.asList("Name", "Total Classes Present", "Total Classes Absent", "Total Classes Tardy", "Total Classes Excused")
        ...
        for (AttendanceSummaryModel model : summaryForSections) {
            for (AttendanceSummaryModel.Entry entry: model.getEntries()) {
                if (!entry.isDropped()) {
                    row = isSimpleAttendance ? Arrays.asList(entry.getStudentName(), entry.getTotalClassesPresent() + "", entry.getTotalClassesMissed() + "", entry.getTotalClassesTardy() + "", + entry.getTotalClassesExcused() + "")
