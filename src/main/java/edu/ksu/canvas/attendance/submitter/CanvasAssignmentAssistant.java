...
public class CanvasAssignmentAssistant {
    ...
    public Error createAssignmentInCanvas(Long courseId, AttendanceAssignment attendanceAssignment, OauthToken adminOauthToken) {
        ...
        try {
            canvasAssignmentOptional = canvasApiWrapper.createAssignment(courseId, assignment, adminOauthToken);
        } catch (IOException e) {
            LOG.error("Error while creating canvas assignment for section: " + attendanceAssignment.getSectionId(), e);
            return new Error("Error while creating canvas assignment for section: " + attendanceAssignment.getSectionId());
        }

        if(canvasAssignmentOptional == null || !canvasAssignmentOptional.isPresent())  {
            LOG.error("Error while creating canvas assignment for section: " + attendanceAssignment.getSectionId());
            return new Error("Error while creating canvas assignment for section: " + attendanceAssignment.getSectionId());
        }
    ...
    private void saveCanvasAssignmentId(Long courseId, Assignment canvasAssignment) {
        ...
        for(AttendanceSection section: sectionList) {
            AttendanceAssignment attendanceAssignment = assignmentService.findBySectionId(section.getSectionId());
    ...
    public Error editAssignmentInCanvas(Long courseId, AttendanceAssignment attendanceAssignment, OauthToken adminOauthToken) {

        Optional<Assignment> canvasAssignmentOptional;
        try {
            canvasAssignmentOptional = canvasApiWrapper.getSingleAssignment(courseId, adminOauthToken, attendanceAssignment.getCanvasAssignmentId().toString());
        } catch (IOException e) {
        ...
        try {
            canvasApiWrapper.editAssignment(courseId.toString(), canvasAssignment, adminOauthToken);
        } catch (IOException e) {
            LOG.error("Error while editing canvas assignment for section: " + attendanceAssignment.getSectionId(), e);
            return new Error("Error while editing canvas assignment for section: " + attendanceAssignment.getSectionId());
        }
        return null;
    }
}
